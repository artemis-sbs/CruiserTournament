...
"""
class TonnageObject(SpaceObject):
    ...
    def __init__(self, name=None, x=None,y=None,z=None,angle=None, race=None,hull=None, size=None, fleet_number=-1):
        self.name = name
        self.x = x
        self.y = y
        self.z = z
    ...
    def score_points(self, sim, surrender):
        print(f"Score {self.hull}:{self.name}")
        if self.race in TonnageObject.POINTS:
            ...
            if self.hull in race:
                ...
                sbs.send_message_to_player_ship(0,"yellow",f'Total tonnage now: {TonnageObject.tonnage}' )
    ...
    def update_state(self, sim):
        sim.delete_navpoint_by_name(self.name);
        if self.state == SpawnState.Spawned:
            if not sim.space_object_exists(self.id):
                ...
                SpaceObject.debug_mark_loc(obj.pos.x, 0,obj.pos.z, self.name, "red")
    ...
    def spawn(self, sim):
        obj = self.make_new_active(sim, 'behav_npcship', self.hull)
        ...
        SpaceObject.debug_mark_loc(sim, self.x, 0,self.z, self.name, "red")
        blob = obj.data_set
        blob.set("name_tag", self.name,0)
        return obj
...
class TonnageHunter(TonnageObject):
    ...
    def prime(self,sim, obj):
        ...
        blob = obj.data_set
        blob.set("throttle", 0.35+self.index*0.01,0)
        blob.set("shield_max_value", 20,0) # front shields
        blob.set("shield_max_value", 20,1) # back shields
    ...
    def score_points(self, sim, surrender):
        # overriding surrender value
        # this feels like smelly code
        if surrender: 
            TonnageObject.tonnage += 20
        else:
            TonnageObject.tonnage += 40
        super().score_points(sim, False)
